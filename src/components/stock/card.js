...
class Card extends React.Component {
    ...
    componentDidUpdate() {
        var ctx = document.getElementById('miniChart_' + this.stock.symbol).getContext('2d');
        var myChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels:  this.props.historic.map(h => h.date),
                datasets: [{
                    label: 'Close',
                    data: this.props.historic.map(h => h.close),
    ...
    render() {
        ...
        return (
            ...
            <a href={"../" + this.stock.source + "/" + this.stock.symbol} >
                <div className="searchBar_card bg-dark p-2 text-white mt-2 rounded">
                    <div className="row">
                        <div className="col-6 display-4 ">{this.stock.symbol}</div>
                        <div className="col-6 ">
                            <div className="searchBar_card_value">$56.00</div>
                            <div className=" badge badge-success ">45.04 (+4.04%) </div>
                        </div>
                    </div>
                    <div className="row m-0 ">
                    <div className="searchBar_card_name col-12 d-flex align-items-center justify-content-center">{this.stock.company_name}</div>
                        <canvas id={"miniChart_" + this.stock.symbol} height="150px" width="334px"></canvas>
                        <div className="searchBar_card_name col-12 d-flex align-items-center justify-content-center text-secondary">Source: {this.stock.source}</div>
                    </div>
                    <div className="row m-0">
...
export default connect(mapStateToProps)(withCookies(Card));
